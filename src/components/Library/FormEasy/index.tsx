...
interface FormEasyItem extends ButtonProps {
  type: string;
  field?: string;
...
export interface FormEasyProps extends FormComponentProps {
  items: FormEasyItem[];
  actions?: ButtonProps[];
  formItemLayout?: any;
  className?: any;
...
class FormEasy extends PureComponent<FormEasyProps, FormEasyState> {
  containerRef: React.RefObject<any>;

  autoHeightList: string[] = ['img', 'upload', 'textArea'];

  constructor(props: Readonly<FormEasyProps>) {
    super(props);
    this.state = {};
    if (props) {
  ...
  getFormElement = (element: FormEasyItem, index: number) => {
    if (!element) {
      return null;
    }
    ...
    switch (type) {
      ...
      case 'custom':
        return (
          <Col key={`element${index}`} span={fill ? 24 : span} style={{ height: colHeight }}>
...
export default Form.create<FormEasyProps>()(FormEasy);
