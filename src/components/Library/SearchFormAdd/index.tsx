...
class SearchFormAdd extends PureComponent<SearchFormProps, SearchState> {
  columnNumOfRow: number;
  ...
  render() {
    ...
    return (
      <Fragment>
        <div className={classNames(styles.searchForm, styles.formWrapper)} ref={this.containerRef}>
  ...
  getFormElement = (element: SearchFormItem, index) => {
    ...
    const { type, field, initialValue, placeholder, children, onChange } = element;
    switch (type) {
      case 'input':
        cpt = (
          <div>
            <span className={styles.colorblack}>单位类型 : </span>
      ...
      case 'select':
        ...
        cpt = (
          <div>
            <span className={styles.colorblack}> 单位名称：</span>
            <Select placeholder={placeholder} onChange={onChange as onSelectChange}>
              {options}
            </Select>
          </div>
        );
...
export default Form.create<SearchFormProps>()(SearchFormAdd);
